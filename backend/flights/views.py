...
def _ensure_pricing_fields(offer, segments):
...
def search_flight(request):
	...
	try:
		...
		for offer in response.data:
			...
			offer_with_links = _ensure_pricing_fields(offer_with_links, segments)
			...
			'departure': _format_datetime(departure.get('at', '')),
			'arrival': _format_datetime(last_segment.get('arrival', {}).get('at', '')),
...
def price_offer(request):
	try:
		...
		for offer in flight_offers:
			...
			cleaned_offers.append(_ensure_pricing_fields(offer, segments))
